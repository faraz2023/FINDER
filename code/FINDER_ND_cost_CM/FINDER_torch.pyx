...
class FINDER:
    ...
    def __init__(self):
        ...
        ############----------------------------- variants for CM 220328 (start) ------------------- ###################################
        # read CM graph, default = HI-II-14
        self.cm_g = nx.read_gml("../../data/real/cost/HI-II-14_degree.gml")

        # get degree list P(k)
        self.cm_degrees = np.array([val for (node, val) in self.cm_g.degree()])
        # normalize P(k) to (0,1]
        self.cm_degrees_mean = np.mean(self.cm_degrees)
        self.cm_degrees = self.cm_degrees / np.max(self.cm_degrees)
        self.cm_degrees, self.cm_degrees_ctr = np.unique(self.cm_degrees, return_counts=True)
        # get frequency of each normalized P(k)
        self.cm_degrees_freq = self.cm_degrees_ctr / np.sum(self.cm_degrees_ctr)


        self.g_type =  'configuration_model'
        ############----------------------------- variants for CM 220328 (end) ------------------- ###################################
    ...
    def gen_graph(self, num_min, num_max):
        ...
        #####----------------------------- variants for CM 220328 (start) ------------------- ###################################
        elif self.g_type == 'configuration_model':
            g = self.cm_graph(cur_n, self.cm_degrees, self.cm_degrees_freq, self.cm_degrees_mean)
        ####----------------------------- variants for CM 220328 (start) ------------------- ###################################
        ### random weight
    ...
    def fit(self,g_list,covered,actions,list_target):
        ...
        for i in range(0,n_graphs,BATCH_SIZE):
            ...
            print("n2nsum_param:" , self.inputs['n2nsum_param'].shape)
            print("laplacian_param:" , self.inputs['laplacian_param'].shape)
            print("subgsum_param:" , self.inputs['subgsum_param'].shape)
